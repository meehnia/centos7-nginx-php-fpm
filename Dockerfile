...
ENV php_fpm_conf /etc/fpm/php-fpm.conf

# Install Basic Requirements
RUN yum -y install epel-release yum-utils vim nano zip unzip \
...
RUN sed -i -e "s/;cgi.fix_pathinfo=1/cgi.fix_pathinfo=0/g" ${php_conf} \
    ...
    && sed -i -e "s/^;pm.process_idle_timeout = 10s;/pm.process_idle_timeout = 5s;/g" ${fpm_conf} \
